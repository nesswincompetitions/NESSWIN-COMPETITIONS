...
const WinnerDetail = () => {
  ...
  const stages = [
    t('winners.detail.stages.selected'),
    t('winners.detail.stages.contacted'),
    t('winners.detail.stages.confirmed'),
    t('winners.detail.stages.prizeSent'),
    t('winners.detail.stages.handoverComplete'),
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-serif font-bold text-white">{t('winners.detail.winnerDetails')}</h1>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
            <div className="space-y-1">
              <p className="text-sm text-gray-500">{t('winners.detail.winnerName')}</p>
            ...
            <div className="space-y-1">
              <p className="text-sm text-gray-500">{t('winners.detail.winningTicket')}</p>
              <p className="text-lg font-mono font-bold text-primary">{winner.ticket}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-500">{t('winners.detail.competition')}</p>
              <p className="text-white font-medium">{winner.competition}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-500">{t('winners.detail.drawDate')}</p>
          ...
          <div className="mt-16 flex justify-center border-t border-white/5 pt-6">
            ...
            >
              {currentStage === stages.length - 1 
                ? t('winners.detail.handoverCompleted') 
                : t('winners.detail.markComplete', { stage: stages[currentStage + 1] })}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            {t('winners.detail.communication')}
          </h2>
            ...
            <div className="p-4 border-t border-white/10 bg-[#121212]/50">
              <div className="mb-3 flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-xs text-gray-400">{t('winners.detail.sendAsEmail')}</span>
                </label>
                <button className="cursor-pointer text-xs text-primary hover:underline flex items-center gap-1">
                  <RefreshCw size={12} /> {t('winners.detail.resendLastEmail')}
                </button>
              </div>
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder={t('winners.detail.messagePlaceholder')} 
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            {t('winners.detail.proofHandover')}
          </h2>
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300 flex items-center justify-between">
                  <span>{t('winners.detail.winnerProof')}</span>
                  <Badge variant="success">{t('winners.detail.uploaded')}</Badge>
                </label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">{t('winners.detail.handoverPhoto')}</label>
                <label className="block border-2 border-dashed border-white/10 rounded-xl p-6 flex flex-col items-center justify-center gap-2 hover:bg-white/[0.02] transition-colors cursor-pointer group">
                  ...
                  <p className="text-sm text-gray-400 mt-1">{t('winners.detail.clickUploadPhoto')}</p>
                </label>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">{t('winners.detail.handoverVideo')}</label>
                <label className="block border-2 border-dashed border-white/10 rounded-xl p-6 flex flex-col items-center justify-center gap-2 hover:bg-white/[0.02] transition-colors cursor-pointer group">
                  <input type="file" accept="video/*" className="hidden" />
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <Video className="text-gray-400" size={20} />
                  </div>
                  <p className="text-sm text-gray-400 mt-1">{t('winners.detail.clickUploadVideo')}</p>
                </label>
              </div>

              <div className="space-y-2 pt-2 border-t border-white/5">
                <label className="text-sm font-medium text-gray-300">{t('winners.detail.instagramLink')}</label>
                <div className="flex gap-2">
